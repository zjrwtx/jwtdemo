...
const Mindmap = () => {
  ...
  const handleCopy = () => {
    navigator.clipboard.writeText(mindmapresult).then(() => {
      message.success('已经复制生成的markdown格式思维导图了！');
    }).catch(() => {
  ...
  return (
    <div style={{ maxWidth: '600px', margin: '0 auto', padding: '20px' }}>
      <Title level={2} style={{ textAlign: 'center', marginBottom: '30px' }}>思维导图生成</Title>
      <Paragraph style={{ textAlign: 'center', marginBottom: '40px' }}>
        不挂科AI：上传一个或多个PDF或PPTX格式的专业课件来生成您的markdown格式思维导图
      </Paragraph>
